...
class Runcible(monome.App):
    ...
    def draw_octave_page(self):
        ...
        self.my_buffer.led_map(0,0,np.roll(np.unpackbits(np.invert((256-np.exp2(np.reshape(4-np.absolut(np.asarray(self.current_track.octave),(16,1))))).astype(np.uint8)),axis=1),0,axis=1))
