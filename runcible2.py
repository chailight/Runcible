...
class Runcible(monome.App):
    ...
    def draw_notes_page(self):
        if self.current_track.track_id == 0:
            ...
            self.my_buffer.led_map(0, 0, np.asarray(self.current_pattern.step_ch2).T)
        elif self.current_track.track_id == 2:
            self.my_buffer.led_map(0, 0, np.asarray(self.current_pattern.step_ch3).T)
        elif self.current_track.track_id == 3:
            self.my_buffer.led_map(0, 0, np.asarray(self.current_pattern.step_ch4).T)
    ...
    def draw_octave_page(self):
        ...
        #self.my_buffer.led_map(0,0,np.roll(np.fliplr(np.unpackbits(np.invert((256-np.exp2(np.reshape(np.asarray(self.current_track.octave),(16,1)))).astype(np.uint8)),axis=1)),4,axis=1))
        self.my_buffer.led_map(0,0,np.roll(np.unpackbits(np.invert((256-np.exp2(np.reshape(np.absolute(np.clip(np.asarray(self.current_track.octave),-3,0)),(16,1)))).astype(np.uint8)),axis=1),-5,axis=1))
